...
public class MonthAdapter extends BaseAdapter {//BaseAdapter를 상속하여 새로운 어댑터 정의
    ...
    private void init() {//1개월의 일별 데이터를 담고 있을 수 있는 MonthItem의 배열 객체 생성
    ...
    public void setPreviousMonth() {//이전 월로 이동 시 일별 데이터로 새로 계산
    ...
    public void setNextMonth() {//다음 월로 이동 시 일별 데이터 새로 계산
        mCalendar.add(Calendar.MONTH, 1);
        recalculate();
    ...
    public void resetDayNumbers() {//지정한 월의 일별 데이터를 새로 계산하는 메소드 정의
        for (int i = 0; i < 42; i++) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // calculate row and column.getView()메소드에서 리턴하는 뷰에 현재날짜의 MonthItem 객체 설정
